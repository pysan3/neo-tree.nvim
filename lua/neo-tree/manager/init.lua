...
function Manager:show_filter(state, search_as_you_type, fuzzy_finder_mode, use_fzy)
  ...
  vim.print(string.format([[search_as_you_type: %s]], vim.inspect(search_as_you_type)))
  vim.print(string.format([[fuzzy_finder_mode: %s]], vim.inspect(fuzzy_finder_mode)))
  vim.print(string.format([[use_fzy: %s]], vim.inspect(use_fzy)))
  vim.print(string.format([[default_value: %s]], vim.inspect(default_value)))
  local input = require("nui.input")(popup_options, {
    prompt = " ",
    default_value = default_value,
    on_submit = function(term)
      vim.print(string.format([[on_submit: value: %s]], vim.inspect(term)))
      if not term or #term == 0 then
        return state:sort_tree_previous()
      elseif search_as_you_type and fuzzy_finder_mode then
        -- end search
      end
      state:cancel_all_tasks("__search")
      state:add_task(
        function()
        ...
        "__search",
        function(...)
          vim.print(string.format([[search canceled(%s): '%s']], { ... }, term))
        end
      )
    end,
    on_change = function(term)
      vim.print(string.format([[on_change: value: %s]], vim.inspect(term)))
      if not search_as_you_type or not term or #term <= 3 then
        return
      ...
      default_value = term -- cache previous query to not redraw on same query
      state:add_task(
        function()
          state:search_tree(self, winwidth, term, use_fzy, types)
        end,
        ...
        function(suc, err)
          ...
          return state._tree_lock.release()
        end
      )
    end,
  })
  ...
  local function end_search()
    vim.cmd.stopinsert()
    input:unmount()
    if fuzzy_finder_mode and state.search_pattern then
      -- TODO: reset search
      state:sort_tree_previous()
    end
  end
  input:map("i", "<Esc>", end_search, { noremap = true })
  input:map("i", "<C-w>", "<C-S-w>")
  input:on({ "BufLeave", "BufDelete" }, end_search, { once = true })
...
function Manager.setup(user_config)
  ...
  if sources[1] ~= "filetree" or #sources ~= 1 then
    log.warn(
      [[TESTING BRANCH. Overwriting source list to just: { "filetree" }. Config options of `filesystem` will be used.]]
    )
    sources = { "filetree" }
    -- local index = 1
